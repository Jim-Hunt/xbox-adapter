...
void SpiDrv::spiSlaveSelect() {
  ...
  // the timeout is only needed for the case when the shield or module is not
  // present
  for (unsigned long start = millis();
       (digitalRead(SLAVEREADY) != HIGH) && (millis() - start) < 5;)
    ;
}
  ...
  while (!waitSlaveReady()) {
    if (feed_watchdog) {
      if ((millis() - start) < 10000) {
        // FIXME: Add support for watchdog
        // WiFi.feedWatchdog();
      }
    }
  }
...
int SpiDrv::waitResponseCmd(uint8_t cmd, uint8_t numParam, uint8_t* param,
                            uint8_t* param_len) {
  char _data = 0;
  int ii = 0;
...
/*
int SpiDrv::waitResponse(uint8_t cmd, uint8_t numParam, uint8_t* param,
uint16_t* param_len)
{
    char _data = 0;
    int i =0, ii = 0;
...
int SpiDrv::waitResponseData16(uint8_t cmd, uint8_t* param,
                               uint16_t* param_len) {
  char _data = 0;
  uint16_t ii = 0;
...
/*
int SpiDrv::waitResponse(uint8_t cmd, tParam* params, uint8_t* numParamRead,
...
int SpiDrv::waitResponse(uint8_t cmd, uint8_t* numParamRead, uint8_t** params,
                         uint8_t maxNumParams) {
  char _data = 0;
  int i = 0, ii = 0;
...
void SpiDrv::sendParamNoLen(uint8_t* param, size_t param_len,
                            uint8_t lastParam) {
  size_t i = 0;
  // Send Spi paramLen
  sendParamLen8(0);
...
uint16_t SpiDrv::readParamLen16(uint16_t* param_len) {
  uint16_t _param_len =
      spiTransfer(DUMMY_DATA) << 8 | (spiTransfer(DUMMY_DATA) & 0xff);
  if (param_len != NULL) {
    *param_len = _param_len;
  }
  return _param_len;
...
void SpiDrv::sendCmd(uint8_t cmd, uint8_t numParam) {
  ...
  if (numParam == 0) spiTransfer(END_CMD);
}

...
void Gamepad::setRumble2(uint8_t l_force, uint8_t r_force, uint8_t duration) const {
  if (!isConnected()) {
    WARN("gamepad not connected");
    return;
  }

  uint8_t rumble[3] = {l_force, r_force, duration};

  WAIT_FOR_SLAVE_SELECT();
  // Send Command
  SpiDrv::sendCmd(BP32_SET_GAMEPAD_RUMBLE_2, PARAM_NUMS_4);
  SpiDrv::sendParam((uint8_t*)&_state.idx, 1, NO_LAST_PARAM);
  SpiDrv::sendParam(rumble, 3, LAST_PARAM);
  ...
  // Wait for reply
  uint8_t data, dataLen;
  if (!SpiDrv::waitResponseCmd(BP32_SET_GAMEPAD_RUMBLE_2, PARAM_NUMS_1, &data,
                               &dataLen)) {
    WARN("error waitResponse");
  }
